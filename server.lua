...
-- Callback: Fetch Player Job Data
...
RegisterNetEvent('multijob:server:changeJob', function(jobName, jobGrade)
    ...
    if target then
        ...
        TriggerClientEvent('ox_lib:notify', src, {title = 'Job Switched', description = 'Clocked into: ' .. target.job_label, type = 'success'})
    end
end)

-- Export: Add Job Experience
exports('AddJobXP', function(source, amount)
    ...
    if d then
        ...
        if (d.job_xp + amount) >= nextXP then
            ...
            TriggerClientEvent('ox_lib:notify', source, {title = 'Level Up!', description = 'You reached Level '..(d.job_level + 1), type = 'success'})
        else
...
RegisterCommand('addjob', function(source, args)
    ...
    local Player = Core.Functions.GetPlayer(src)
    if not Player or not Config.AdminGroups[Player.PlayerData.permission] then return end
    ...
    if jobInfo then
        local gradeLabel = jobInfo.grades[tostring(jobGrade)] and jobInfo.grades[tostring(jobGrade)].name or "Staff"
        ...
        TriggerClientEvent('ox_lib:notify', targetId, {title = 'Career Access', description = 'Unlocked: ' .. jobInfo.label, type = 'inform'})
    end
end)